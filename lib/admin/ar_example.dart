...
class CloudAnchorWidget extends StatefulWidget {
  const CloudAnchorWidget(
      {Key? key, required this.hint, required this.locationHint})
      : super(key: key);
...
class _CloudAnchorWidgetState extends State<CloudAnchorWidget> {
  ...
  Future<void> onNodeTapped(List<String> nodeNames) async {
    ...
    arSessionManager!.onError(foregroundNode.data!["hint"]);
  }
    ...
    if (singleHitTestResult != null) {
      ...
      if (didAddAnchor ?? false) {
        ...
        var newNode = ARNode(
          ...
          uri:
              "https://github.com/kuosc2005/3dobjects/raw/main/minecraft_chest%20_hq.glb",
  ...
  onAnchorUploaded(ARAnchor anchor) {
    ...
    firebaseManager.uploadAnchor(
      ...
      locationHint: widget.locationHint,
      objectHint: widget.hint,
    );
  ...
  Future<void> onDownloadButtonPressed() async {
    ...
    if (this.arLocationManager!.currentLocation != null) {
      firebaseManager.downloadAnchorsByLocation((snapshot) {
        ...
        arAnchorManager!.downloadAnchor(cloudAnchorId).then((value) {
          log(value.toString());
        });
...
class FirebaseManager {
  ...
  Future<bool> initializeFlutterFire() async {
    try {
      ...
      // await Firebase.initializeApp(Defa)
      geo = GeoFlutterFire();
  ...
  void uploadAnchor(ARAnchor anchor,
    ...
    if (currentLocation != null) {
      ...
      serializedAnchor['locationHint'] = locationHint;
      serializedAnchor['objectHint'] = objectHint;
    }
