...
class WoodenChickenObjectWidget extends StatefulWidget {
  @override
  WoodenChickenObjectWidgetState createState() =>
      WoodenChickenObjectWidgetState();
}

class WoodenChickenObjectWidgetState extends State<WoodenChickenObjectWidget> {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Visibility(
                    ...
                    child: Row(
                      children: [
                        ...
                        //     child: const Text("Measure Distance → Field"),
                        //   ),
                        // ),
                      ],
                    ),
                  ...
                  //         child: ElevatedButton(
                  //           style: translucentButtonStyle,
                  //           onPressed: () => _scaleSelected(-0.1),
                  //           child: const Text("Scale - (Smaller)"),
                  //         ),
                  //       ),
                  //       const SizedBox(width: 8),
                  //       Expanded(
  ...
  Future<void> onPlaceObjectButtonPressed(String objectType) async {
    ...
    if (objectType == 'chicken') {
      newNode = ARNode(
        ...
        //uri: "Models/Biyo/biyo.gltf",
        scale: vm.Vector3(0.2, 0.2, 0.2),
        position: vm.Vector3(0.0, 0.0, 0.0),
        rotation: vm.Vector4(1.0, 0.0, 0.0, 0.0),
      );
    } else {
      // NEW MODEL ANALYSIS (updated biyor.gltf):
      //
      // Model has origin at the TIP (Y=0), field extends to Y=-100
      // X-axis centered (±57 units), Z-axis flat (~0)
      //
      // After glTF export, with 90° Y rotation:
      // - Model's -Y becomes -Z in AR (away from camera)
      // - Tip (Y=0) stays at origin
      // - Field extends in -Z direction (away from user)
      //
      // NO OFFSET NEEDED - origin is already at the tip!
      //
      const double groundOffsetX = 0.0;
      const double groundOffsetZ = 0.0;

      newNode = ARNode(
        type: NodeType.localGLTF2,
        uri: "Models/Biyorfield/biyor.gltf",
        scale: vm.Vector3(0.2, 0.2, 0.2),
        // No position offset needed - model origin is at the field tip
        position: vm.Vector3(groundOffsetX, 0.0, groundOffsetZ),
        // Rotation: 90° around Y-axis to orient field away from camera
        // Quaternion (0, sin(45°), 0, cos(45°)) = (0, 0.7071, 0, 0.7071)
        rotation: vm.Vector4(0.0, 0.7071067811865476, 0.0, 0.7071067811865476),
      );
    }
  ...
  /// Scale the selected object by a delta amount (e.g., 0.1 or -0.1)
  /// Positive delta = increase size, Negative delta = decrease size
  Future<void> _scaleSelected(double delta) async {
    if (selectedIndex == null) {
    ...
    // Get current scale
    final currentScale = node.scale;

    // Calculate new scale (ensure minimum scale of 0.05 to prevent disappearing)
    final newScaleValue = (currentScale.x + delta).clamp(0.05, 5.0);

    // Apply uniform scaling on all axes
    final newScale = vm.Vector3(newScaleValue, newScaleValue, newScaleValue);

    // Assign new scale
    node.scale = newScale;

    // Force notifier update to reflect changes
    node.transformNotifier.value = node.transformNotifier.value;

    print(
        "📐 Scaled selected object. New scale: ${newScaleValue.toStringAsFixed(2)}");
    setState(() {});
  }

  final ButtonStyle translucentButtonStyle = ElevatedButton.styleFrom(
    backgroundColor: woodBrown.withOpacity(0.30),
    foregroundColor: whiteColor,
    elevation: 0,
    shape: RoundedRectangleBorder(
      borderRadius: BorderRadius.circular(10),
      side: const BorderSide(
        color: borderColor, // neon cyan
        width: 1.2,
      ),
    ),
  );
